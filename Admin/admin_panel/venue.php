...
<body>
	...
	<div class="container-fluid">
		<div class="row">
			<h2 style="text-align: left; font-size: 40px; ">Booked Venues</h2>
...
$(document).ready(function() {
  $.ajax({
    url: 'booked2_events.php?date=today',
    type: 'GET',
    dataType: 'json',
    success: function(response) {
      if (response.status == 'success') {
        ...
        for (var i = 0; i < bookings.length; i++) {
          ...
          rows += '<td><a href="#" class="btn btn-danger btn-sm" style = "background-color: #019a02;border-color: #007bff;"">' + booking.venue_type + '</td>';
          rows += '<td>' + booking.participants + '</td>';
          rows += '<td>' + booking.date + ' <br> ' + booking.time + '</td>';
          rows += '<td>' + booking.org_name + '</a></td>';
